...
public sealed class PluginLoader
{
		...
		plugins.ForEach(tuple =>
		{
			...
			try
			{
				...
				foreach (var type in assembly.GetExportedTypes())
				{
					...
					var folder = Path.Join(Eldora.PluginPath, metadata.PluginName);
					...
					typeof(AbstractPlugin).GetField(nameof(plugin.MainWindow))?.SetValue(plugin, Eldora.Instance.MainForm);
					typeof(AbstractPlugin).GetField(nameof(plugin.PlatformHandler))?.SetValue(plugin, Eldora.Instance.PlatformHandler);
	...
	{
		foreach (var plugin in _loadedPlugins) action((plugin.Metadata, plugin));
	}
